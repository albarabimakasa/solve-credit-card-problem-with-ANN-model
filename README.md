...
#### Features Selection
Removing unused variables. ```python
...
fromkeras.layers import Dense, Dropout
...
#### Model Visualization
```python
fromkeras.utils.vis_utils import plot_model
...
X_test and y_test data y_test is data that has never been seen by the ANN model. ```python
...
number_of categories = 2
target_names = ['class{}'.format(i) for i in range(number_of_categories)]
print(classification_report(y_test, prediction_results,target_names=target_names))
```
...
plt.show( )
```
![accuracy under sampling](https://raw.githubusercontent.com/albarabimakasa/solve-credit-card-problem-with-ANN-model/main/picture/accuracy%20under%20sampling.png)
...
weighted avg 0.93 0.92 0.92 89 By using the undersampling technique, the F1-Score value was obtained as 0.92. We ignore the perfect precision value in Class1 and focus on the F1-Score value, this is because [F1-Score](https://en.wikipedia.org/wiki/F1_score) itself is the harmonic mean of precision and recall.

## Under Testing Sampling on final X_test
This test is running the model on data that has never been seen before. or data that represents the real world. ```python
